...
use Redismq::RedisScript::Unpop;
use Moo;

#
# description:
# the class controls the number of assigned messages to limit it to a max value
#
# how it works:
# use a redis key (like __MaxPopTRQueue_pop_lock_<message>) to count the
# number of assigned messages
# on a pop event:
# 1st - if assigned messages lower than max do a pop action and return it
# 2nd - if not, check the assigned for timed out messages
...
has 'unpop_s'
  => (
      is => 'lazy',
      default => sub { Redismq::RedisScript::Unpop->new(
							redis => $_[0]->redis
						       );
		     },
     );
